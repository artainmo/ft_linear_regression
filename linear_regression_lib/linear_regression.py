...
#Used to facilitate calculations with matrices, theta_0 are y-intercept is added as value one
...
#When using a function of degree 1, our prediction line can only be straight
...
#predicted values, are the values you predicted with your current algorithm
    def __init__(self, thetas, alpha=0.0001, n_cycle=1000000, lambda_=0):
        ...
        #because gradient descend uses the slope that naturally goes towards 0 when coming close to the local minma, gradient descend can by itself modulate the stepsize
        #But still when alpha is too small algorithm needs to perform more steps and become slower
        #And when alpha is too big no convergence(finding the local minima) will occur, because it will "overshoot" the local minima
        ...
        #Too much can slow down the algorithm and too few and the algorithm won't converge
        #The ideal n_cycle and alpha id dpendent on the dataset and needs to be tested
