...
LR = linear_regression(thetas=np.zeros((x_values.shape[1] + 1, 1)), n_cycle=8000000)
LR.plot_(data[0], LR.predict_(data[0]), data[1], LR.cost_(LR.predict_(data[0]), data[1])) #Initial prediction line
print('Cost before training: ' + str(LR.cost_(LR.predict_(data[0]), data[1])[0][0]))
LR.fit_(data[0], data[1])
print('Cost after training: ' + str(LR.cost_(LR.predict_(data[0]), data[1])[0][0]))
